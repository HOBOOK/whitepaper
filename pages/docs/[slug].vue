      ...
      <article class="card p-8">
        <div v-if="isEditing">
          <div class="flex items-center gap-2 mb-3">
            <button :class="tabClass('editor')" @click="viewTab='editor'">Editor</button>
            <button :class="tabClass('preview')" @click="viewTab='preview'">Preview</button>
          </div>

          <div v-if="viewTab==='editor'">
            <textarea v-model="raw" class="w-full h-[60vh] rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm font-mono"></textarea>
          </div>
        ...
        <div v-if="admin" class="mt-4 flex gap-2">
          <button class="btn" @click="startEdit" v-if="!isEditing">Edit</button>
        </div>
      </article>
      <aside>
...
function tabClass(name){
  return 'px-3 py-1.5 rounded-lg text-sm ' + (viewTab.value===name ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200')
}
...
async function saveEdit(){
  try{
    ...
    doc.value = updated
  }catch(e){ saveError.value = (e && e.data && e.data.message) || 'Save failed' }
}
